...
CHILD="/hpc/group/igvf/K562/leave-one-out/BlueSTARR/mutator-child.py"
...
class CrePosition:
    def __init__(self,text):
        if(not rex.find(r"(\d+):ref=(.):([^,]+),([^,]+),([^,]+)(?:,([^,]+))?",text)):
            raise Exception("Can't parse pos: "+text)
        self.pos=int(rex[1])
        self.ref=rex[2]
        self.alleles=[rex[3],rex[4],rex[5]]
        if rex[6] is not None: self.alleles.append(rex[6])
...
def runModel(recs,seqLen,fastaFile,inputFile,outputFile,JOBSIZE,model):
    ...
    while(True):
        ...
        (defline,seq)=pair
        (actualInterval,attr)=FastaReader.parseDefline(defline) # actualInterval: seq retrieved from 2bittofa
        cre=recs[creIndex]
        chrom=cre.chrom
        (globalBegin,globalEnd)=cre.window # begin and end for actualInterval
...
"""
Pipe is kind of redundant right now, subprocess is a better to fork out the process in python
One problem is that pipe won't show the print output from the child 
even setting flush to True didn't solve the issue
Revathy has to change Pipe to subprocess into debuggning the code in child
"""
...
"""
Split into chunks -> TempFile -> run -> concatenate to the final file -> rm TempFile
when the input file is large, one case Revathy experienced is that the final output 
only has the final chunk, because each is overwritten by the next chunk
Reason: the tempFile keep output the same name -> overwrite
Solution by Revathy: tempFile with different names 

Note: this is the problem that account for 60% of issues Revathy encountered 
"""
