...
def pwm_to_logodds(arr_pwm_WxB, arr_bg_B=(0.25, 0.25, 0.25, 0.25), eps=1e-6):
    """
    ...
    arr_bg_B    = np.clip(np.asarray(arr_bg_B,    dtype=float), eps, 1.0)

    ### normalize background to ensure it sums to 1
    arr_bg_B /= arr_bg_B.sum()

    ### calculate log-odds score
    arr_lod_WxB = np.log2(arr_pwm_WxB / arr_bg_B[None, :])
    return arr_lod_WxB