...
def one_hot_encode(txt_seq, txt_alphabet = "ACGT"):
    """
    one hot encoding of sequence (L,) to matrix (L,B) 
    ...
    -------
    np.ndarray
        matrix of one-hot encoded sequence, with shape (L,B)
        Default: shape = (L,4)
    """
...
def decode_one_hot(arr_seq_LxB, txt_alphabet = "ACGT"):
    """Helper function to turn one-hot encoded matrix back to seuqence"""
    return "".join(txt_alphabet[idx] for idx in arr_seq_LxB.argmax(axis=1))
